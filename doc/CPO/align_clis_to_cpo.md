# Align CLIs to display CPO fields | Common and Vendor-Specific
...
to include the new CPO data in the output.

It includes 2 main parts:
1. Ensure we print the common new CPO fields - using new key formatting map for these fields.
2. Create new ability to have key formatting map for the vendor-specific fields.
...
The goal is simple: **CPO/ELS fields must be formatted consistently across CLIs**,
while keeping vendor-specific formatting logic in platform code.

## 4. Requirements

- Ensure we print the common new CPO fields - using new key formatting map for these fields.
- Create new ability to have key formatting map for the vendor-specific fields.
- **CPO/ELS fields must be formatted consistently across CLIs**,
while keeping vendor-specific formatting logic in platform code.
- `sonic-utilities` should not hardcode vendor-specific labels/units.
Platform implementations (CPO \ SFP platform classes) provide their own maps via the APIs (5.3).
...
### 5.2. Add common CPO/ELS format maps in utilities (sfputil and sfpshow)

Add/extend CPO-specific display maps in shared utility code for common fields (not vendor-private fields), including:

- CPO transceiver info labels (`els_*` info keys)
- CPO DOM labels/units (`els_*` DOM keys)
- CPO status labels where applicable

These maps are merged into the existing flows only when the transceiver type is CPO.

### 5.3. Add explicit vendor-specific formatting APIs on `SfpBase`

Expose vendor hooks as three explicit APIs:

- `get_vendor_specific_transceiver_info_format_map() -> dict`
- `get_vendor_specific_transceiver_status_format_map() -> dict`
- `get_vendor_specific_dom_format_map() -> tuple(dict, dict)`

Default implementation returns empty maps, so platforms with no vendor-specific extension continue to work without behavior change.

### 5.4. Keep vendor-specific map ownership in platform code

`sonic-utilities` should not hardcode vendor-private labels/units.
Platform implementations (for example, CPO \ SFP platform classes) provide their own maps via the APIs above.
...
for key in info:
    if key in vendor_specific_dom_format_map:
